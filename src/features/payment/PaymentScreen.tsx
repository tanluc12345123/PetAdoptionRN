...
import {PaymentRouteProp} from '../../navigation/route/index';
import {PaymentRootProps} from '../../navigation/props/index';
import {BaseScreen} from '../../components/BaseScreen';
import ToolbarHeader from '../../components/ToolbarHeader/ToolbarHeader';
import {Drawables} from '../../assets/images/index';
import {PaymentStyles} from './Payment.Style';
...
const PaymentScreen = () => {
  ...
  const navigation = useNavigation<PaymentRootProps>();
  const {isLoading, data, booking, error} = usePayment();
  ...
  const dataDetail = {
    ...
    transactions: [
      {
        amount: {
          total: route.params.bookingAppointment.pet.price,
          currency: 'THB',
          details: {
            subtotal: route.params.bookingAppointment.pet.price,
            tax: '0',
            shipping: '0',
            handling_fee: '0',
            shipping_discount: '0',
        ...
        .then(response => {
          ...
          if (response.data.payer.status === 'VERIFIED') {
            Alert.alert('Payment!', 'Payment Successful!', [
              {text: 'OK', onPress: () => bookingAppointment(true)},
            ]);
          } else {
  ...
  const bookingAppointment = (payment: boolean) => {
    let form = new FormData();
    form.append('visitingDate', route.params.bookingAppointment.visitingDate);
    form.append('visitingTime', route.params.bookingAppointment.visitingTime);
    form.append('payment', payment);
    booking({
      body: form,
      user: route.params.bookingAppointment.user,
    ...
    <BaseScreen
      ...
      {showGateway && approvalUrl && (
        <Modal
          ...
          animationType={'fade'}
          transparent>
          <View style={PaymentStyles.webViewCon}>
            <View style={PaymentStyles.wbHead}>
              <TouchableOpacity
                style={{padding: 13}}
